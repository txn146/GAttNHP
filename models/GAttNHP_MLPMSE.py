...
class AttNHPDyad_GroupTime(nn.Module):
    def __init__(self, n_entity, n_rel, n_groups, group_map_tensor, config):
        ...
        # A. Event Intensity Head
        self.intensity_layer = nn.Sequential(
            nn.Linear(self.d_feature, n_entity),
            nn.Softplus()
        )
        
        # B. Time Prediction Head (Simple MLP for MSE)
        # Output dimension is 1 (Scalar dt)
        self.time_predictor = nn.Sequential(
            nn.Linear(self.d_feature, 64),
            nn.ReLU(),
            nn.Linear(64, 1),
            nn.Softplus() # Ensure dt > 0
        )
        
        # Initialize bias to start with small predictions
        with torch.no_grad():
            # The last linear layer is at index -2 (before Softplus)
            self.time_predictor[-2].bias.fill_(-5.0)
    ...
    def forward(self, subs, marks, objs, times, dt, mask):
        ...
        # MSE Prediction (Single Value)
        # Output shape: [B, L-1, 1] -> squeeze to [B, L-1]
        dt_pred = self.time_predictor(enhanced_features).squeeze(-1)
        
        return lambda_t, dt_pred