...
from .layers import AttNHP, MLPNonCrossingQuantile
...
class AttNHPDyad_GroupTime(nn.Module):
    def __init__(self, n_entity, n_rel, n_groups, group_map_tensor, config):
        ...
        # B. Time Prediction Head (Using MLP)
        # This is the key difference from the standard GAttNHP
        self.quantile_func = MLPNonCrossingQuantile(
            d_context=self.d_feature,
            quantiles=[0.025, 0.25, 0.5, 0.75, 0.95],
            hidden_size=64
        )
    ...
    def forward(self, subs, marks, objs, times, dt, mask):
        ...
        # 2. Core Encoding
        # Pass hist_dt so the Encoder learns the rhythm
        ...
        # Predict Quantiles using MLP Head
        all_quantiles = self.quantile_func(enhanced_features)
        
        return lambda_t, all_quantiles